                ...
                <div class="col-md-12">
                    <div class="card strpied-tabled-with-hover">
                        <div class="card-header ">
                            ...
                            <form class="form-inline  pull-right my-2 my-lg-0" method="POST" action="{{'search_time'}}">
                                @csrf
                                <input class="form-control mr-sm-2" type="search" name="search_name"
                                    placeholder="Search_name" aria-label="Search">
                                <div class="form-group row">
                                    <label for="example-month-input" class="col-sm-2 col-form-label">date</label>
                                    <div class="col-sm-10">
                                        <input class="form-control mr-sm-2" type="date" value="วว/ดด/ปป"
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input class="form-control mr-sm-2" type="month" value="วว/ดด/ปป"
                                            name="search_month" id="example-month-input">
                                    </div>
                                </div>
                            ...
                            $mo_pdf = empty($month) ? '' : $month;
                            @endphp

                            @if($name_pdf != "" || $date_pdf != "" )


                            @if($name = "$name_pdf" and $month = "$mo_pdf")

                            <a target="_blank" href="/pdf/{{$name_pdf}}/{{$mo_pdf}}" class="btn btn-outline-info"
                                role="button" aria-pressed="true">PDF</a>

                            @elseif ($date = "$date_pdf")

                            <a target="_blank" href="/date_pdf/{{$date}}" class="btn btn-outline-info" role="button"
                                aria-pressed="true">PDF</a>

                            @elseif ($month = "$mo_pdf")

                            <a target="_blank" href="/date_pdf/{{$mo_pdf}}" class="btn btn-outline-info" role="button"
                                aria-pressed="true">PDF</a>

                            @elseif ($name = $name_pdf)

                            <a target="_blank" href="/name_pdf/{{$name}}" class="btn btn-outline-info" role="button"
                                aria-pressed="true">PDF</a>

                            @endif
                            @else
                            <a target="_blank" href="{{url('pdf2') }}" data-toggle="modal" data-target="#exampleModal"
                                class="btn btn-outline-info" role="button" aria-pressed="true">PDF</a>
